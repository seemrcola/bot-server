...
export class ReActExecutionStep implements ChainStep {
  ...
  async *execute(context: ChainContext): AsyncIterable<string> {
    ...
    const { maxSteps = 8, strategy = config.reactStrategy } = context.options;
    ...
    if (strategy === 'function') {
      ...
      try {
        for await (const step of executor.run(context.messages, { maxSteps })) {
        ...
        for await (const step of promptExecutor.run(context.messages, { maxSteps })) {
          reactResults.push(step);
          if (context.options.reactVerbose) {
            yield step + '\n';
          }
